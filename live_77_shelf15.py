#!/usr/bin/env python3
"""PINNED 7/7 shelf15. Dual UNPLUGGED. DEMO SIM ON.

Do not overwrite this file with sniper. Copy to live_77.py on the box.

Fire on the CLOSED 1m that tagged the last-webhook rail:
  hold the side + close within 15 pts + 5m tape with the trade.
Skip / off_shelf do NOT mute the sit. Next 1m can still fire.
Spent only after a real send. Lock 120s then clear. BE off.
No visit_dead. No WAIT_C2. No volume gate. No Dual.

Pine name is the side:
  H4L H1L PDL PWL → bounce long (close OVER)
  H4H H1H PDH PWH → fade short  (close UNDER)
  H4 / H1 → LOCK from first 1m close of the visit
...
ARM_PTS = 15.0  # close must be within 15 of the rail
...
def main():
    ...
    emit(event="seven_start", fire=FIRE, book=BOOK, note=NOTE, symbol=SYMBOL,
         ...
         lock="120s_then_clear", fire_mode="close_1m")
    ...
    while True:
        ...
        if now - last_hb > 60:
            ...
            emit(event="heartbeat", session=ok, why=why, fire=FIRE,
                 ...
                 rails=len(rails), symbol=SYMBOL, spent=len(spent), arm_pts=ARM_PTS)
            last_hb = now
        ...
        rec = dict(
            ...
            submit=False, dual="UNPLUGGED", symbol=SYMBOL, fire_mode="close_1m",
        )
