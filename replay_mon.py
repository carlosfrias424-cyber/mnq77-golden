#!/usr/bin/env python3
"""Monday 9/14. Unique paper_fire + live submit → 20/40 on seven.jsonl mids.
Hole if first mid >8 from entry. Sequential one position. Dual unused.
"""
...
T0 = datetime(2026, 9, 14, 4, 0, tzinfo=CDT)
T1 = datetime(2026, 9, 14, 11, 30, tzinfo=CDT)
...
def load_seven():
    ...
    for ln in p.open():
        ...
        if first is None:
            first = dt
        last = dt
        if dt < T0 or dt > T1 + timedelta(hours=6):
            continue
        out.append((dt, o))
    out.sort(key=lambda x: x[0])
    print("seven.jsonl span", first, "->", last)
    return out
...
def unique_fires(rows):
    ...
    for dt, o in rows:
        if dt < T0 or dt >= T1:
            continue
        ev = o.get("event")
        if ev not in ("paper_fire", "struct40_submit") and not o.get("submit"):
            continue
...
def main():
    print("VERSION mon_seven_mids_20_40  9/14 04:00-11:30")
    print("Path on seven.jsonl mids. Dual off. Live not touched.\n")
    rows = load_seven()
    fires = unique_fires(rows)
    print("unique GOs", len(fires), "seven rows in window", len(rows))
    if not fires:
        print("NO FIRES — Monday not in this log (likely starts Wed 9/16)")
        return
